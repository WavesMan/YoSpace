...
const AdminPostsPage = async () => {
  ...
  return (
    <div className={styles.postsRoot}>
      ...
      {posts.length === 0 ? (
        <p className={styles.postsEmpty}>暂无文章，请先通过“新建文章”创建内容。</p>
      ) : (
          ...
          <tbody>
            {posts.map((post: {
              ...
              <tr key={post.id}>
                ...
                <td className={`${styles.postsCell} ${styles.postsCellMuted}`}>{post.publishedAt.toISOString().slice(0, 10)}</td>
