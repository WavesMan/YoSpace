...
import SettingsClientCore from "./SettingsClientCore";
 ...
 * @returns 页面节点
 */
const SettingsClient: React.FC<SettingsClientProps> = ({ initialAdminPath }) => {
  ...
  const groupedItems = useMemo(() => {
    const site = items.filter(item => item.key.startsWith("NEXT_PUBLIC_SITE_") || item.key.startsWith("NEXT_PUBLIC_NAV_"));
    const blog = items.filter(item => item.key.startsWith("NEXT_PUBLIC_BLOG_") || item.key === "NEXT_PUBLIC_USE_DB_CONTENT");
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      setItems(list);
      setDraftValues(
        Object.fromEntries(list.map(item => [item.key, toInputValue(item.value)])),
      );
    } catch (requestError) {
