 ...
 * @returns 页面 JSX 节点
 */
const AdminAnalyticsPostsPage = async ({ searchParams }: AdminAnalyticsPostsPageProps) => {
  const query = await searchParams;
  const pageQuery = normalizeAdminPageQuery(query);
  const result = await getAdminPostViewsPage(pageQuery);
  ...
  return (
    <div className={styles.analyticsRoot}>
      <div className={styles.analyticsHeader}>
        <div>
          <h1 className={styles.analyticsTitle}>全部文章阅读量</h1>
          <p className={styles.analyticsSubtitle}>按阅读量与更新时间排序的完整文章列表。</p>
        </div>
        <div className={styles.analyticsPager}>
          <Link className={styles.analyticsBackLink} href={`${adminPath}/analytics/paths`}>
            访问路径
          </Link>
      ...
      <section className={styles.analyticsPanel}>
        ...
        {result.items.length === 0 ? (
          <p className={styles.analyticsEmpty}>暂无文章数据。</p>
        ) : (
...
export default AdminAnalyticsPostsPage;